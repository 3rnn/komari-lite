...
func TestDefaultDeploymentProfileUsesSafeInstallationDefaults(t *testing.T) {
	profile := defaultDeploymentProfile(models.Client{})
	if !profile.DisableWebSSH {
		t.Fatal("remote control must be disabled by default")
	}
	if !profile.DisableAutoUpdate {
		t.Fatal("automatic updates must be disabled by default")
	}
	if profile.IgnoreUnsafeCert {
		t.Fatal("unsafe TLS certificates must not be ignored by default")
	}
}
