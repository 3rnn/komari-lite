...
func TestSaveClientInfoIgnoresFieldsUnknownToThePanel(t *testing.T) {
	...
	require.NoError(t, db.Create(&models.Client{UUID: "client-unknown", Token: "token-unknown", Name: "unknown"}).Error)

	require.NoError(t, saveClientInfo(db, map[string]interface{}{
		"uuid":                   "client-unknown",
		"os":                     "Debian GNU/Linux 13 (trixie)",
		"arch":                   "amd64",
		"mem_total":              int64(1014341632),
	...
	require.NoError(t, db.Where("uuid = ?", "client-unknown").First(&stored).Error)
	assert.Equal(t, "Debian GNU/Linux 13 (trixie)", stored.OS)
	assert.Equal(t, "amd64", stored.Arch)
...
func TestDeleteClientRollsBackRelatedRowsWhenClientDeleteFails(t *testing.T) {
	...
	require.NoError(t, db.AutoMigrate(
		...
		&models.LoadNotification{}, &models.MetricCleanupJob{},
	))
