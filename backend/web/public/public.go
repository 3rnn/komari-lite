...
func replaceHTMLLanguage(htmlStr, language string) string {
	...
	replacements := []struct {
		...
		{`<html lang='en'>`, `<html lang='` + language + `'>`},
		{`<html lang="zh-CN">`, `<html lang="` + language + `">`},
		{`<html lang='zh-CN'>`, `<html lang='` + language + `'>`},
...
// EnsureBundledThemes guarantees that the fixed local Glass theme exists. Once
// installed, its local files are never replaced by later application builds.
func EnsureBundledThemes() error {
	// The monitoring-only build has exactly one public theme. Historic settings
	// that selected legacy or third-party themes are reset to the fixed Glass copy.
	currentTheme := DefaultTheme
	if err := os.RemoveAll(filepath.Join(DataDir, ThemesDir, RetiredThemeID)); err != nil {
		return fmt.Errorf("remove retired %s theme: %w", RetiredThemeID, err)
	}
	if !IsLocalThemeUsable(DefaultTheme) {
		if err := installEmbeddedTheme(bundledThemeRoot, DefaultTheme); err != nil {
			return fmt.Errorf("install bundled %s theme: %w", DefaultTheme, err)
		}
	}
