...
| 命名空间 | 方法（文件） |
| --- | --- |
| `admin` | client CRUD、client deployment、ping task、session/settings/weight、notification（load/offline/traffic）、provider（messageSender/oidc）、dashboard（settings/alerts/cache/latency/packet loss）、metric、database、preferences、system（logs/test） |
...
二进制/流/重定向/特殊鉴权类，集中在 `web/api/admin`（2fa/theme settings/backup/update/oauth/https/traffic calibration/download）、
`web/api/public`（login/logout/oauth/mjpeg、agent distribution+installers）、`web/api/client`（report WS+POST、v2 RPC、uploadBasicInfo、runtime config、AutoDiscovery 注册）。
